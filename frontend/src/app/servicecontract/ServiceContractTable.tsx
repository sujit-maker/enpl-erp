...
const ServiceContractTable: React.FC = () => {
  ...
  const fetchContracts = async () => {
    try {
      const response = await axios.get(
        "http://192.168.29.225:8000/servicecontracts"
      );
  ...
  const handleCreate = async () => {
    try {
      await axios.post("http://192.168.29.225:8000/servicecontracts", formData);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        `http://192.168.29.225:8000/servicecontracts/${selectedContract.id}`,
        updatedFormData
      );
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await axios.delete(`http://192.168.29.225:8000/servicecontracts/${id}`);
