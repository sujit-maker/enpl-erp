...
const MaterialDeliveryForm: React.FC = () => {
  ...
  useEffect(() => {
    axios
      .get("http://192.168.29.225:8000/customers")
      .then((res) => setCustomers(res.data));
    axios
      .get("http://192.168.29.225:8000/vendors")
      .then((res) => setVendors(res.data));
    axios
      .get("http://192.168.29.225:8000/products")
      .then((res) => setProducts(res.data));
    axios
      .get("http://192.168.29.225:8000/inventory")
      .then((res) => setInventory(res.data));
    fetchDeliveries(); // Fetch deliveries on component mount
  }, []);

  useEffect(() => {
    axios.get("http://192.168.29.225:8000/inventory").then((res) => {
  ...
  const fetchDeliveries = async () => {
    const res = await axios.get("http://192.168.29.225:8000/material-delivery");
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit) {
        await axios.put(
          `http://192.168.29.225:8000/material-delivery/${formData.id}`,
          payload
        );
        alert("Delivery updated sucessfully!");
      } else {
        await axios.post("http://192.168.29.225:8000/material-delivery", payload);
  ...
  const handleDelete = (id: any): void => {
    if (confirm("Are you sure you want to delete this delivery?")) {
      axios
        .delete(`http://192.168.29.225:8000/material-delivery/${id}`)
        .then(() => {
