...
const paginatedInventory = sortedInventory.slice(
  ...
  const fetchData = async () => {
    try {
      const [inventoryRes, paymentsRes] = await Promise.all([
        axios.get("http://192.168.29.225:8000/inventory"),
        axios.get("http://192.168.29.225:8000/vendor-payment"),
      ]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(
          `http://192.168.29.225:8000/inventory/${formData.id}`,
          payload
        );
  ...
  const handleDelete = async (id?: number) => {
    ...
    await axios.delete(`http://192.168.29.225:8000/inventory/${id}`);
    fetchData();
  };
