...
const VendorPaymentTable: React.FC = () => {
  ...
  const fetchPayments = async () => {
    const res = await axios.get("http://192.168.29.225:8000/vendor-payment");
  ...
  const handleSave = async () => {
    try {
      if (formData.id) {
        await axios.put(
          `http://192.168.29.225:8000/vendor-payment/${formData.id}`,
          formData
        );
        alert("Payment updated sucessfully!");
      } else {
        await axios.post("http://192.168.29.225:8000/vendor-payment", formData);
  ...
  const handleDelete = async (id: number) => {
    ...
    if (confirmed) {
      try {
        await axios.delete(`http://192.168.29.225:8000/vendor-payment/${id}`);
                  ...
                  onSelect={async (val) => {
                    ...
                    try {
                      const res = await axios.get(
                        `http://192.168.29.225:8000/inventory?vendorId=${val}`
                      );
