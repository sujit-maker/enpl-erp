...
const CustomerTable: React.FC = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(
          `http://192.168.29.225:8000/customers/${formData.id}`,
          form,
          {
        ...
        await axios.post("http://192.168.29.225:8000/customers", form, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
      ...
      {filteredCustomers.map((cust) => (
        <tr key={cust.id} className="border-b">
          ...
          <td className="p-2 border text-blue-900">
            {cust.gstpdf ? (
              <a
                href={`http://192.168.29.225:8000/gst/${cust.gstpdf}`}
                target="_blank"
            ...
            <div className="overflow-auto max-h-[90vh]">
                ...
                <div className="mt-4">
                  ...
                  {!gstpdfFile && existingGstFileName && (
                    <a
                      href={`http://192.168.29.225:8000/gst/${existingGstFileName}`}
                      target="_blank"
                      rel="noopener noreferrer"
