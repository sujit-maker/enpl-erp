...
const ServiceSubCategoryTable: React.FC = () => {
  ...
  const fetchSubCategories = async () => {
    try {
      const response = await axios.get("http://192.168.29.225:8000/servicesubcategory");
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this subcategory?")) {
      try {
        await axios.delete(`http://192.168.29.225:8000/servicesubcategory/${id}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selectedSubCategory) {
        await axios.put(`http://192.168.29.225:8000/servicesubcategory/${selectedSubCategory.id}`, {
          serviceSubCatId,
          categoryId,
          subCategoryName,
        });
        ...
        await axios.post("http://192.168.29.225:8000/servicesubcategory", {
          serviceSubCatId,
