...
export default function TicketPage() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      const res = await axios.get(
        `http://192.168.29.225:8000/tickets/user/${userId}`
      );
  ...
  const fetchCustomers = async () => {
    const res = await axios.get("http://192.168.29.225:8000/customers");
  ...
  const fetchTicketDetails = async (id: number) => {
    const res = await axios.get(`http://192.168.29.225:8000/tickets/${id}`);
    setSelectedTicket(res.data);
    const msgRes = await axios.get(`http://192.168.29.225:8000/message/${id}`);
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    await axios.post("http://192.168.29.225:8000/tickets", {
      ...form,
      createdBy: userId,
      assignedTo: 61,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Send message
      await axios.post("http://192.168.29.225:8000/message", {
        content: newMessage,
        senderId: userId,
      ...
      if (
        ...
        await axios.patch(`http://192.168.29.225:8000/tickets/${selectedTicket.id}`, {
          status: "IN_PROGRESS",
        });
      }
  ...
  useEffect(() => {
    ...
    axios.get(`http://192.168.29.225:8000/users/${userId}`).then((res) => {
                        ...
                        {openMenuId === ticket.id && (
                          ...
                          >
                            {getNextStatus(ticket.status) && (
                              <div
                                onClick={async () => {
                                  await axios.patch(`http://192.168.29.225:8000/tickets/${ticket.id}`, {
