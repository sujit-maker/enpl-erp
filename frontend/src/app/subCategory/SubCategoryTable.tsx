...
const SubCategoryTable: React.FC = () => {
  ...
  const handleDelete = async (subCategoryId: number) => {
    if (window.confirm("Are you sure you want to delete this subcategory?")) {
      try {
        await axios.delete(
          `http://192.168.29.225:8000/subcategory/${subCategoryId}`
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selectedSubCategory) {
        ...
        await axios.put(
          `http://192.168.29.225:8000/subcategory/${selectedSubCategory.id}`,
          {
            categoryId,
            subCategoryName,
        ...
        await axios.post("http://192.168.29.225:8000/subcategory", {
          categoryId,
          subCategoryName,
          subCategoryId: newSubCategoryId,
        });
